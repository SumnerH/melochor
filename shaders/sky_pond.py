...
float pondMusicRippleHeight(vec2 position) {
    ...
    for (int index = 0; index < 8; ++index) {
        ...
        // A short-lived wave packet: one impact at the note onset, followed by
        // naturally fading ripples instead of sustained surface pressure.
        float envelope = exp(-wavefront * wavefront * 8.5)
            * exp(-ripple.z * 1.05);
        float irregularity = 0.90 + 0.10 * sin(
            atan(delta.y, delta.x) * 7.0 + ripple.z * 5.0 + surfaceNoise * 8.0
        );
        height += sin(wavefront * 40.0 - ripple.z * 9.0 + surfaceNoise * 3.0)
            * envelope
            * irregularity
            * ripple.w
            * 0.032;
...
float pondTreeLayer(vec2 uv, float horizon, float scale, float darkness) {
    ...
    float canopyHeight = 0.12
        + pondFbm(vec2(uv.x * scale * 0.45, 2.7 + scale)) * 0.20
        + pondNoise(vec2(floor(uv.x * scale * 2.0), scale)) * 0.065;
    ...
    float foliage = pondFbm(
        vec2(uv.x * scale * 3.2, uv.y * 38.0 + scale * 4.0)
    );
...
vec3 renderPondSky(vec2 vPos, vec3 baseColor) {
    ...
    float woodland = max(farTrees, max(middleTrees, nearTrees));
    vec3 treelineColor = mix(
        vec3(0.075, 0.12, 0.13),
        vec3(0.018, 0.070, 0.045),
        nearTrees
    );
    color = mix(color, treelineColor, woodland);
    ...
    // Define the pond as a single irregular shoreline field. Its broad outline
    // remains pond-like, while low-frequency angular variation creates coves and
    // headlands without the layered procedural bands of the previous approach.
    ...
    float broadShore = pondNoise(vec2(shoreAngle * 1.25, 4.7)) - 0.5;
    float coveShore = pondNoise(vec2(shoreAngle * 4.7, 12.3)) - 0.5;
    float shorelineRadius = 1.0 + broadShore * 0.19 + coveShore * 0.065;
    float ellipseDistance = length(pondLocal);
    float shoreDistance = ellipseDistance - shorelineRadius;
    float pondMask = 1.0 - smoothstep(-0.020, 0.028, shoreDistance);

    // Retain these values for the water and vegetation calculations below.
    float pondNormalizedY = pondLocal.y;
    float pondHalfWidth = pondScale.x * shorelineRadius;
    float pondVerticalMask = 1.0;
    float shoreVariation = broadShore * 0.025 + coveShore * 0.012;
    float texturedShoreDistance = shoreDistance;

    // Soften the transition between the far marsh and treeline while retaining
    // the pond's own sharply recognizable organic shoreline.
    ...
    // Rain-darkened marsh grass and mud outside the pond.
    ...
    // A restrained wet-soil transition follows the same shoreline field. It is
    // deliberately subtle: transparent shallows reveal sediment under water,
    // rather than painting concentric mud and vegetation rings around the pond.
    float bottomTexture = pondFbm(uv * vec2(15.0, 19.0));
    vec3 pondBottom = mix(
        vec3(0.075, 0.115, 0.070),
        vec3(0.17, 0.18, 0.105),
        bottomTexture
    );
    vec3 wetGrass = mix(
        vec3(0.045, 0.085, 0.040),
        vec3(0.105, 0.135, 0.060),
        bottomTexture
    );
    float bankWetness = 1.0 - smoothstep(0.015, 0.135, shoreDistance);
    marshGround = mix(marshGround, wetGrass, bankWetness * 0.48);

    float rushSeed = pondNoise(vec2(
        floor(uv.x * 96.0),
        floor(vPos.y * 54.0)
    ));
    float rushes = smoothstep(0.84, 0.94, rushSeed)
        * smoothstep(0.075, -0.015, shoreDistance);
    marshGround = mix(marshGround, vec3(0.025, 0.105, 0.045), rushes * 0.32);
    ...
    // Shallow water becomes translucent in stages: the organic mud texture is
    // visible beneath it before the damp bank and grass take over outside.
    float waterShallows = smoothstep(0.62, 1.00, ellipseDistance / shorelineRadius);
    ...
    // Let the pond dissolve gradually into its visible shallow bottom and wet
    // bank. This gives the water a transparent, natural edge rather than an
    // opaque ellipse cut sharply into the surrounding land.
    float pondWaterOpacity = pondMask * (
        1.0 - smoothstep(0.84, 1.02, ellipseDistance / shorelineRadius)
    );
    color = mix(color, water, pondWaterOpacity);
